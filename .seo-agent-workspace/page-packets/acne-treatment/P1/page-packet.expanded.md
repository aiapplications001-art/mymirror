# Page Packet: acne-treatment Category Page

## Meta Requirements

- Title: acne-treatment Category Page | MyMirror
- Description: acne-treatment Category Page for IndianSkinCareSeekers, with clear guidance, references, and one primary action path.
- Slug: acne-treatment
- H1: acne-treatment Category Page
- Primary keyword: Not provided
- Secondary keywords: None provided
- OG title: acne-treatment Category Page | MyMirror
- OG description: acne-treatment Category Page for IndianSkinCareSeekers, with clear guidance, references, and one primary action path.
- Twitter title: acne-treatment Category Page | MyMirror
- Twitter description: acne-treatment Category Page for IndianSkinCareSeekers, with clear guidance, references, and one primary action path.

## Authorship

- Authored by: MyMirror Editorial Team
- Reviewed by visibility: hidden
- Created: 2026-08-16
- Updated: 2026-08-16

## CTA

- Primary CTA: Start your free skin analysis now
- CTA microcopy: Scan your face to understand visible acne, oiliness, marks, texture, and skin signals in about 60 seconds.
- Destination: Not provided
- Mobile sticky CTA: Start analysis

<!-- SECTION_ID: S1_hero -->
## Hero

# acne-treatment Category Page

If you are trying to understand acne-treatment category page, start with the visible signals you can act on today. MyMirror helps you move from concern to a clearer next step with one focused path: Start your free skin analysis now.

**Start your free skin analysis now**

Scan your face to understand visible acne, oiliness, marks, texture, and skin signals in about 60 seconds.

<!-- SECTION_ID: S2_quick_answer -->
## Quick Answer

acne-treatment Category Page is designed for readers who want a practical, source-aware answer on acne-treatment category page. The page should explain what the concern means, what signals matter, and when the reader should move toward the recommended action.

<!-- SECTION_ID: S3_problem_diagnosis -->
## Problem Diagnosis

Expanded review-ready draft copy for Problem Diagnosis. Replace this with adapter-written final prose when live source review and editorial approval are complete.

<!-- SECTION_ID: S4_selection_criteria -->
## Selection Criteria

Expanded review-ready draft copy for Selection Criteria. Replace this with adapter-written final prose when live source review and editorial approval are complete.

<!-- SECTION_ID: S5_solution_mapping -->
## Solution Mapping

Expanded review-ready draft copy for Solution Mapping. Replace this with adapter-written final prose when live source review and editorial approval are complete.

<!-- SECTION_ID: S6_common_mistakes -->
## Common Mistakes

Expanded review-ready draft copy for Common Mistakes. Replace this with adapter-written final prose when live source review and editorial approval are complete.

<!-- SECTION_ID: S7_trust_proof -->
## Trust Proof

Show why this page can be trusted: include the author, creation or update date, review method where relevant, and a short methodology note. Keep reviewer visibility hidden unless the editor explicitly chooses otherwise.

<!-- SECTION_ID: S8_faq -->
## Faq

### Frequently Asked Questions

**What should I know before choosing acne-treatment category page?**

Start with visible signals, your goal, and whether you need education, comparison, or direct product guidance.

**Can this page include FAQ schema?**

Yes, if these questions remain visible on the page and are reviewed with the final copy.

<!-- SECTION_ID: S9_final_cta -->
## Final Cta

Ready for the next step? Start your free skin analysis now.

Scan your face to understand visible acne, oiliness, marks, texture, and skin signals in about 60 seconds.

<!-- SECTION_ID: S10_references -->
## References

Reference URLs still need live source review. Add official, brand, competitor, or citation URLs here only after checking relevance, source quality, and external-link purpose.


## Internal Links

- None

## External References

- Add cited reference URLs after live research.

## Image Slots

| ID | Section | Purpose | Aspect ratio | Status |
| --- | --- | --- | --- | --- |
| IMG_OG | reserved | Open Graph image for social and search previews. | 1.91:1 | reserved |
| IMG_HERO | S1_hero | Mandatory hero visual for the first fold. | 16:9 | brief_needed |
| IMG_02 | S2_quick_answer | High-impact visual support for S2_quick_answer. | 4:3 | brief_needed |
| IMG_03 | S3_problem_diagnosis | High-impact visual support for S3_problem_diagnosis. | 4:3 | brief_needed |
| IMG_04 | S4_selection_criteria | High-impact visual support for S4_selection_criteria. | 4:3 | brief_needed |

## JSON-LD Drafts

```json
[
  {
    "type": "WebPage",
    "jsonLd": {
      "@context": "https://schema.org",
      "@type": "WebPage",
      "name": "acne-treatment Category Page",
      "description": "acne-treatment Category Page for IndianSkinCareSeekers, with clear guidance, references, and one primary action path."
    }
  },
  {
    "type": "BreadcrumbList",
    "jsonLd": {
      "@context": "https://schema.org",
      "@type": "BreadcrumbList",
      "itemListElement": [
        {
          "@type": "ListItem",
          "position": 1,
          "name": "Home",
          "item": "/"
        },
        {
          "@type": "ListItem",
          "position": 2,
          "name": "acne-treatment",
          "item": "/acne-treatment"
        }
      ]
    }
  },
  {
    "type": "FAQPage",
    "jsonLd": {
      "@context": "https://schema.org",
      "@type": "FAQPage",
      "mainEntity": []
    }
  }
]
```

## Machine-Readable JSON

```json
{
  "metadata": {
    "schemaVersion": "page-packet.v1",
    "sourcePrewritingPageId": "P1",
    "companyName": "MyMirror",
    "market": "India",
    "pageType": "product_category",
    "createdDate": "2026-08-16",
    "updatedDate": "2026-08-16",
    "copyStatus": "expanded_review_ready"
  },
  "seo": {
    "title": "acne-treatment Category Page | MyMirror",
    "description": "acne-treatment Category Page for IndianSkinCareSeekers, with clear guidance, references, and one primary action path.",
    "slug": "acne-treatment",
    "h1": "acne-treatment Category Page",
    "secondaryKeywords": [],
    "ogTitle": "acne-treatment Category Page | MyMirror",
    "ogDescription": "acne-treatment Category Page for IndianSkinCareSeekers, with clear guidance, references, and one primary action path.",
    "twitterTitle": "acne-treatment Category Page | MyMirror",
    "twitterDescription": "acne-treatment Category Page for IndianSkinCareSeekers, with clear guidance, references, and one primary action path."
  },
  "authorship": {
    "author": {
      "name": "MyMirror Editorial Team"
    },
    "reviewedByVisible": false
  },
  "cta": {
    "primary": {
      "label": "Start your free skin analysis now",
      "microcopy": "Scan your face to understand visible acne, oiliness, marks, texture, and skin signals in about 60 seconds."
    },
    "mobileSticky": {
      "label": "Start analysis"
    }
  },
  "rendering": {
    "mobileFirst": true,
    "desktopNotes": [
      "Keep the primary CTA visible in the first fold.",
      "Use section blocks as editable units."
    ],
    "mobileNotes": [
      "Use a sticky shortened CTA where appropriate.",
      "Prioritize short paragraphs and visible section hierarchy."
    ]
  },
  "sections": [
    {
      "id": "S1_hero",
      "heading": "Hero",
      "role": "conversion",
      "markdown": "# acne-treatment Category Page\n\nIf you are trying to understand acne-treatment category page, start with the visible signals you can act on today. MyMirror helps you move from concern to a clearer next step with one focused path: Start your free skin analysis now.\n\n**Start your free skin analysis now**\n\nScan your face to understand visible acne, oiliness, marks, texture, and skin signals in about 60 seconds."
    },
    {
      "id": "S2_quick_answer",
      "heading": "Quick Answer",
      "role": "seo",
      "markdown": "acne-treatment Category Page is designed for readers who want a practical, source-aware answer on acne-treatment category page. The page should explain what the concern means, what signals matter, and when the reader should move toward the recommended action."
    },
    {
      "id": "S3_problem_diagnosis",
      "heading": "Problem Diagnosis",
      "role": "ux",
      "markdown": "Expanded review-ready draft copy for Problem Diagnosis. Replace this with adapter-written final prose when live source review and editorial approval are complete."
    },
    {
      "id": "S4_selection_criteria",
      "heading": "Selection Criteria",
      "role": "seo",
      "markdown": "Expanded review-ready draft copy for Selection Criteria. Replace this with adapter-written final prose when live source review and editorial approval are complete."
    },
    {
      "id": "S5_solution_mapping",
      "heading": "Solution Mapping",
      "role": "conversion",
      "markdown": "Expanded review-ready draft copy for Solution Mapping. Replace this with adapter-written final prose when live source review and editorial approval are complete."
    },
    {
      "id": "S6_common_mistakes",
      "heading": "Common Mistakes",
      "role": "ux",
      "markdown": "Expanded review-ready draft copy for Common Mistakes. Replace this with adapter-written final prose when live source review and editorial approval are complete."
    },
    {
      "id": "S7_trust_proof",
      "heading": "Trust Proof",
      "role": "trust",
      "markdown": "Show why this page can be trusted: include the author, creation or update date, review method where relevant, and a short methodology note. Keep reviewer visibility hidden unless the editor explicitly chooses otherwise."
    },
    {
      "id": "S8_faq",
      "heading": "Faq",
      "role": "seo",
      "markdown": "### Frequently Asked Questions\n\n**What should I know before choosing acne-treatment category page?**\n\nStart with visible signals, your goal, and whether you need education, comparison, or direct product guidance.\n\n**Can this page include FAQ schema?**\n\nYes, if these questions remain visible on the page and are reviewed with the final copy."
    },
    {
      "id": "S9_final_cta",
      "heading": "Final Cta",
      "role": "conversion",
      "markdown": "Ready for the next step? Start your free skin analysis now.\n\nScan your face to understand visible acne, oiliness, marks, texture, and skin signals in about 60 seconds."
    },
    {
      "id": "S10_references",
      "heading": "References",
      "role": "reference",
      "markdown": "Reference URLs still need live source review. Add official, brand, competitor, or citation URLs here only after checking relevance, source quality, and external-link purpose."
    }
  ],
  "links": {
    "internal": [],
    "external": []
  },
  "images": [
    {
      "id": "IMG_OG",
      "purpose": "Open Graph image for social and search previews.",
      "aspectRatio": "1.91:1",
      "altText": "acne-treatment Category Page preview image",
      "status": "reserved"
    },
    {
      "id": "IMG_HERO",
      "sectionId": "S1_hero",
      "purpose": "Mandatory hero visual for the first fold.",
      "aspectRatio": "16:9",
      "altText": "acne-treatment hero visual",
      "status": "brief_needed"
    },
    {
      "id": "IMG_02",
      "sectionId": "S2_quick_answer",
      "purpose": "High-impact visual support for S2_quick_answer.",
      "aspectRatio": "4:3",
      "altText": "acne-treatment quick answer visual",
      "status": "brief_needed"
    },
    {
      "id": "IMG_03",
      "sectionId": "S3_problem_diagnosis",
      "purpose": "High-impact visual support for S3_problem_diagnosis.",
      "aspectRatio": "4:3",
      "altText": "acne-treatment problem diagnosis visual",
      "status": "brief_needed"
    },
    {
      "id": "IMG_04",
      "sectionId": "S4_selection_criteria",
      "purpose": "High-impact visual support for S4_selection_criteria.",
      "aspectRatio": "4:3",
      "altText": "acne-treatment selection criteria visual",
      "status": "brief_needed"
    }
  ],
  "schemaDrafts": [
    {
      "type": "WebPage",
      "jsonLd": {
        "@context": "https://schema.org",
        "@type": "WebPage",
        "name": "acne-treatment Category Page",
        "description": "acne-treatment Category Page for IndianSkinCareSeekers, with clear guidance, references, and one primary action path."
      }
    },
    {
      "type": "BreadcrumbList",
      "jsonLd": {
        "@context": "https://schema.org",
        "@type": "BreadcrumbList",
        "itemListElement": [
          {
            "@type": "ListItem",
            "position": 1,
            "name": "Home",
            "item": "/"
          },
          {
            "@type": "ListItem",
            "position": 2,
            "name": "acne-treatment",
            "item": "/acne-treatment"
          }
        ]
      }
    },
    {
      "type": "FAQPage",
      "jsonLd": {
        "@context": "https://schema.org",
        "@type": "FAQPage",
        "mainEntity": []
      }
    }
  ],
  "machineReadable": {
    "sections": [
      {
        "id": "S1_hero",
        "heading": "Hero",
        "role": "conversion",
        "markdown": "# acne-treatment Category Page\n\nIf you are trying to understand acne-treatment category page, start with the visible signals you can act on today. MyMirror helps you move from concern to a clearer next step with one focused path: Start your free skin analysis now.\n\n**Start your free skin analysis now**\n\nScan your face to understand visible acne, oiliness, marks, texture, and skin signals in about 60 seconds."
      },
      {
        "id": "S2_quick_answer",
        "heading": "Quick Answer",
        "role": "seo",
        "markdown": "acne-treatment Category Page is designed for readers who want a practical, source-aware answer on acne-treatment category page. The page should explain what the concern means, what signals matter, and when the reader should move toward the recommended action."
      },
      {
        "id": "S3_problem_diagnosis",
        "heading": "Problem Diagnosis",
        "role": "ux",
        "markdown": "Expanded review-ready draft copy for Problem Diagnosis. Replace this with adapter-written final prose when live source review and editorial approval are complete."
      },
      {
        "id": "S4_selection_criteria",
        "heading": "Selection Criteria",
        "role": "seo",
        "markdown": "Expanded review-ready draft copy for Selection Criteria. Replace this with adapter-written final prose when live source review and editorial approval are complete."
      },
      {
        "id": "S5_solution_mapping",
        "heading": "Solution Mapping",
        "role": "conversion",
        "markdown": "Expanded review-ready draft copy for Solution Mapping. Replace this with adapter-written final prose when live source review and editorial approval are complete."
      },
      {
        "id": "S6_common_mistakes",
        "heading": "Common Mistakes",
        "role": "ux",
        "markdown": "Expanded review-ready draft copy for Common Mistakes. Replace this with adapter-written final prose when live source review and editorial approval are complete."
      },
      {
        "id": "S7_trust_proof",
        "heading": "Trust Proof",
        "role": "trust",
        "markdown": "Show why this page can be trusted: include the author, creation or update date, review method where relevant, and a short methodology note. Keep reviewer visibility hidden unless the editor explicitly chooses otherwise."
      },
      {
        "id": "S8_faq",
        "heading": "Faq",
        "role": "seo",
        "markdown": "### Frequently Asked Questions\n\n**What should I know before choosing acne-treatment category page?**\n\nStart with visible signals, your goal, and whether you need education, comparison, or direct product guidance.\n\n**Can this page include FAQ schema?**\n\nYes, if these questions remain visible on the page and are reviewed with the final copy."
      },
      {
        "id": "S9_final_cta",
        "heading": "Final Cta",
        "role": "conversion",
        "markdown": "Ready for the next step? Start your free skin analysis now.\n\nScan your face to understand visible acne, oiliness, marks, texture, and skin signals in about 60 seconds."
      },
      {
        "id": "S10_references",
        "heading": "References",
        "role": "reference",
        "markdown": "Reference URLs still need live source review. Add official, brand, competitor, or citation URLs here only after checking relevance, source quality, and external-link purpose."
      }
    ],
    "images": [
      {
        "id": "IMG_OG",
        "purpose": "Open Graph image for social and search previews.",
        "aspectRatio": "1.91:1",
        "altText": "acne-treatment Category Page preview image",
        "status": "reserved"
      },
      {
        "id": "IMG_HERO",
        "sectionId": "S1_hero",
        "purpose": "Mandatory hero visual for the first fold.",
        "aspectRatio": "16:9",
        "altText": "acne-treatment hero visual",
        "status": "brief_needed"
      },
      {
        "id": "IMG_02",
        "sectionId": "S2_quick_answer",
        "purpose": "High-impact visual support for S2_quick_answer.",
        "aspectRatio": "4:3",
        "altText": "acne-treatment quick answer visual",
        "status": "brief_needed"
      },
      {
        "id": "IMG_03",
        "sectionId": "S3_problem_diagnosis",
        "purpose": "High-impact visual support for S3_problem_diagnosis.",
        "aspectRatio": "4:3",
        "altText": "acne-treatment problem diagnosis visual",
        "status": "brief_needed"
      },
      {
        "id": "IMG_04",
        "sectionId": "S4_selection_criteria",
        "purpose": "High-impact visual support for S4_selection_criteria.",
        "aspectRatio": "4:3",
        "altText": "acne-treatment selection criteria visual",
        "status": "brief_needed"
      }
    ],
    "links": {
      "internal": [],
      "external": []
    }
  }
}
```
